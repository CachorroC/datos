...
function newJuzgado(
  ...
  for ( const proceso of procesos ) {
    ...
    const matchedDespacho = despachos_1.Despachos.find(
      (
        despacho 
      ) => {
...
async function createCarpetasDemanda() {
  ...
  for ( const carpeta of carpetas_1.default ) {
    const index = carpetas_1.default.indexOf(
      carpeta 
    );
    ...
    console.log(
      `carpetas has a length of ${ carpetas_1.default.length } and you are in carpeta number ${ carpeta.numero }` 
    );
  ...
  const insertCarpetas = await ( 0, __1.insertNewCarpetas )(
    newCarpetasArray 
  );
  console.log(
    insertCarpetas 
