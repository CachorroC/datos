...
export  async function fetchProceso( {
  ...
  try {
    ...
    if ( !req.ok ) {
      ...
      fs.writeFile(
        `src/carpetas/${ index }/notOkprocesos.json`, JSON.stringify( req )
      );
    ...
    fs.writeFile(
      `src/carpetas/${ index }/procesos.json`, JSON.stringify( res )
    );
...
export function fixJuzgado( tipo: string ) {
  const shortened = tipo.search( /[A-Z]+[A-Z]/g );

  if ( shortened === -1 ) {
    return tipo;
  }
  const isPromiscuo = tipo.search( /(PM)/g );
  const isCircuito = tipo.search( /(CC)/g );

  const isCircuitoEjecucion
    = tipo.search( /(CCTOE)/g );
  const isDesgloce = tipo.search( /(DESG)/g );
  const isPequenasCausas = tipo.search( /(PCC)/g );

  if ( isCircuitoEjecucion > 0 ) {
    return 'Civil del Circuito de Ejecucion';
  }

  if ( isPequenasCausas > 0 ) {
    return ' Pequeñas Causas y Competencias Multiples';
  }

  if ( isCircuito > 0 ) {
    return 'Civil del Circuito';
  }

  if ( isPromiscuo > 0 ) {
    return 'Promiscuo Municipal';
  }
...
export async function transformRawtoCarpeta ( {
  carpetaRaw, index
}: { carpetaRaw: IntCarpetaRaw; index? : number } ): Promise<IntCarpeta> {
  const juzgadosMap: Map<number, Juzgado> = new Map();
  ...
  fs.mkdir(
    `src/carpetas/${ newCarpeta.numero }`, {
      recursive: true
    }
  );
  fs.writeFile(
    `src/carpetas/${ newCarpeta.numero }/carpeta.json`, JSON.stringify( newCarpeta )
  );

  return newCarpeta;
}

export async function mapCarpetas ( {
  carpetas
  ...
  console.log( newCarpetasMap.size );
