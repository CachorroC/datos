...
export default async function fetchProceso( {
...
}: { llaveProceso: string; index: number;  id: number} ) {
  ...
  if ( llaveProceso.length < 23 || llaveProceso === 'sin especificar' ) {
    return [];
  }
  ...
  try {
    ...
    if ( !req.ok ) {
      ...
      fs.writeFile(
        `./carpetas/${ id }/notOkprocesos.json`, JSON.stringify( req )
      );
    ...
    fs.writeFile(
      `./carpetas/${ id }/procesos.json`, JSON.stringify( res )
    );
    ...
    fs.writeFile(
      `./carpetas/${ id }/networkError.json`, JSON.stringify( error )
    );
