...
async function fetchProcesos(llaveProceso, index = 1, _id = 'unknown') {
    ...
    try {
        if (llaveProceso.length < 23
            ...
            throw new Error(`la llaveProceso es menor de 23: ${llaveProceso}`);
        }
        ...
        if (!req.ok) {
            throw new Error(`la peticion de procesos salió erronea, registra: ${req.status}: ${req.statusText}`);
        }
