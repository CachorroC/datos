...
async function getProcesosbyLLaveProceso() {
    ...
    const llavesLength = exports.llavesProceso.length;
    ...
    */
    const procesosArray = exports.llavesProceso.map(async (llave, index) => {
        const procesos = await (0, procesos_1.default)(llave.llaveProceso, index, llave._id);
        return procesos;
    });
    fs.writeFile('procesosbyidsleepInfetch.json', JSON.stringify(procesosArray));
