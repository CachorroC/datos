...
exports.trimmer = exports.juzgadoFixer = exports.telFixer = exports.carpetas = void 0;
...
function telFixer(tel) {
    const fijo = new Map();
    const celular = new Map();
    const hasFijo = tel.includes('F');
    if (hasFijo) {
        const indexOfFijo = tel.indexOf('F');
        const fijoRaw = tel
            .slice(indexOfFijo, 10)
            .replace('F ', '');
        fijo.set('fijo', fijoRaw);
    }
    const hasCel = tel.includes('CEL');
    if (hasCel) {
        const indexOfCel = tel.indexOf('CEL');
        const celularRaw = tel
            .slice(indexOfCel)
            .replace('CEL ', '');
        celular.set('celular', celularRaw);
    }
    return {
        fijo: Array.from(fijo.values()).toLocaleString(),
        celular: Array.from(celular.values()).toLocaleString()
    };
}
exports.telFixer = telFixer;
function juzgadoFixer(juzgado) {
    if (juzgado.id) {
        const id = juzgado.id;
        const tipo = juzgado.tipo.replace(/\d/g, '');
        return {
            ...juzgado,
            id: id,
            tipo: tipo
    ...
    const id = juzgado.tipo.replace(/\D/g, '');
    const tipo = juzgado.tipo.replace(/\d/g, '');
    return {
        ...juzgado,
        id: id,
...
const newMapCarpetas = exports.carpetas.map((carpeta) => {
    if (carpeta.deudor.tel) {
        telFixer(carpeta.deudor.tel.toString());
    }
    const juzgados = carpeta.demanda?.juzgado?.ejecucion
        ?? carpeta.demanda?.juzgado?.origen;
    console.log(juzgados);
    const trimmedName = trimmer(carpeta.deudor.nombreCompleto);
    const { nombreCompleto, ...newDeudor } = carpeta.deudor;
    const obligaciones = [];
    if (carpeta.demanda) {
        obligaciones.push(carpeta.demanda.obligacion);
        if (carpeta.demanda.obligacion2) {
            obligaciones.push(carpeta.demanda.obligacion2);
        }
    }
    const newCarpeta = {
        ...carpeta,
        demanda: {
            ...
            juzgado: {
                origen: juzgadoFixer(carpeta.demanda?.juzgado?.origen ?? {
                    tipo: 'unknown',
                    id: 0,
                    url: 'https://app.rsasesorjuridico.com'
                }),
                ejecucion: juzgadoFixer(carpeta.demanda?.juzgado?.ejecucion ?? {
                    tipo: 'unknown',
                    id: 0,
                    url: 'https://app.rsasesorjuridico.com'
                })
            },
            despachoActivo: juzgadoFixer(juzgados ?? {
                tipo: 'unknown',
                id: 0,
                url: 'https://app.rsasesorjuridico.com'
            })
        ...
        deudor: {
            ...carpeta.deudor,
            ...trimmedName,
            tel: telFixer(carpeta.deudor.tel?.toString() ?? 'nada')
        }
    };
    return newCarpeta;
});
console.log(newMapCarpetas);
fs.writeFile('nuevosNombresMap.json', JSON.stringify(newMapCarpetas));
