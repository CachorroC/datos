import { ZodCarpetas } from './data/zod-carpetas';
...
export const DemandaSchema = z.object({
  ...
  expediente: z.string(),
  juzgados: z.array(JuzgadoSchema)
});
...
export const IntCarpetaElementSchema = z.object({
  ...
  lastModified: z
    ...
    .optional(),
  ultimaActuacion: z
    .union([UltimaActuacionSchema, z.null()])
    .optional()
});
...
export function idkCarpetas() {
  for (const carpeta of ZodCarpetas) {
    const parser =
      IntCarpetaElementSchema.parse(carpeta);
    console.log(parser);
    fs.writeFile(
