...
async function createCarpetasDemanda() {
  ...
  for ( const rawCarpeta of carpetas_raw_1.default ) {
    const indexOfCarpeta
      = carpetas_raw_1.default.indexOf(
        rawCarpeta 
      );

    const carpeta = ( 0,
    categories_1.categoryAssignment )(
      rawCarpeta 
    );
    ...
    console.log(
      `carpetas has a length of ${ carpetas_raw_1.default.length } and you are in carpeta number ${ indexOfCarpeta }`
    );
    ...
    if ( RequestProcesos ) {
      const newCarpeta = new carpeta_1.Carpeta(
        carpeta,
        RequestProcesos
      );
      newCarpetas.add(
